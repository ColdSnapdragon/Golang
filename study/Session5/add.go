...
func def() {
	defer fmt.Println(1) // defer语句的作用是在函数执行完毕后执行一些清理操作，无论函数是正常返回还是发生了panic
	defer func() {
		/* 如果在deferred函数(defer的运行时真身)中调用了内置函数recover，当函数发生了panic异常，recover会使程序从panic中恢复，并返回panic value。导致panic异常的函数不会继续运行，但能正常返回。
	...
	// 当函数发生panic时，会先执行defer语句，然后才会中断当前的执行流程并传播panic。
	// defer语句逆序执行
}
